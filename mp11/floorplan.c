...
/*
 * This file contains several functions that will be called by main.c
 * and they are defined in the header file floorplan.h
 * I wrote is_leaf_node function, is_internal_node, is_in_subtree, rotate, recut, swap_module, swap_topology, get_expression, init_slicing_tree, get_total_resource
 */
...
int is_leaf_node(node_t* ptr) {
	...
	if (ptr == NULL) return 0;	// check if it's NULL
	else if (!(ptr->left) && !(ptr->right)) return 1;	// if both left or right ptr have NULL, it is a leaf
	else return 0;	// otherwise, it's not a leaf
}
...
int is_internal_node(node_t* ptr) {
	// TODO: (remember to modify the return value appropriately)
	if (ptr == NULL) return 0;	// check if it's NULL
	else if (is_leaf_node(ptr)) return 0;	// if the node is a leaf, then it's not an internal node
	else return 1;	// otherwise, it's an internal node
}
...
int is_in_subtree(node_t* a, node_t* b) {
	// TODO: (remember to modify the return value appropriately)
	// traverse the tree
	if (a == NULL || b == NULL) return 0;	// reach end
	if (a == b) return 1;	// b is found
...
void rotate(node_t* ptr) {
	// TODO: 
	if (is_leaf_node(ptr)) {	// swap w and h
...
void recut(node_t* ptr) {
	...
	// TODO:
	if (ptr->cutline == V) ptr->cutline = H;		// if it was V, change to H
	else if (ptr->cutline == H) ptr->cutline = V;	// if it was H, change to V
}
...
void swap_module(node_t* a, node_t* b) {
	...
	// TODO:
	// swap two nodes' module attribute
...
void swap_topology(node_t* a, node_t* b) {
	...
	// TODO:
	node_t * tmp_a = a;	// assign a to a tmp node
	node_t * b_parent = b->parent;	// assign b->parent to b_parent
	
	// hook b to a's position
	if (a->parent->left == a) {
		a->parent->left = b;
	...
	// hook a to b's position
	if (b_parent->left == b) {
		b_parent->left = tmp_a;
		tmp_a->parent = b_parent;
	} else if (b_parent->right == b)  {
		b_parent->right = tmp_a;
		tmp_a->parent = b_parent;
	}
}
...
int get_total_resource(node_t* ptr){
	// TODO:
	int sum=0;	// initlize sum to 0
	int i;
	for (i=0; i<num_modules; i++) sum += modules[i].resource;	// add all resources from nodes
...
node_t* init_slicing_tree(node_t* par, int n) {
	...
	// TODO:
	// base case: last modules 
	if(n+1 == num_modules){
		// make left node
		node_t * ptr_left = (node_t *) malloc(sizeof(node_t));		// allocate space for left node
		ptr_left->module = modules+n;
		ptr_left->cutline = UNDEFINED_CUTLINE;
		ptr_left->left = NULL;
		ptr_left->right = NULL;
		// hook them up
		ptr_left->parent = par;
	...
	// general case
	// make parent module
	node_t * ptr_parent = (node_t *) malloc(sizeof(node_t));	// allocate space for right node
	ptr_parent->module = NULL;
	ptr_parent->cutline = V;

	// make right module
	node_t * ptr_right = (node_t *) malloc(sizeof(node_t));	// allocate space for right node
	ptr_right->module = modules+n;
	ptr_right->cutline = UNDEFINED_CUTLINE;
	ptr_right->left = NULL;
	ptr_right->right = NULL;
	
	// hook up right module
	ptr_right->parent = ptr_parent;
	ptr_parent->right = ptr_right;
	ptr_parent->parent = par;
	
	// hook up left node recursively
	ptr_parent->left = init_slicing_tree(ptr_parent, n+1);
